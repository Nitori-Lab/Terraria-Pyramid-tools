...
This module implements platform-specific operations for Windows systems.
"""
...
class WindowsPlatform(PlatformAdapter):
    ...
    Handles TerrariaServer interaction using PowerShell/Windows commands.
    """
    ...
    def find_terraria_server(self) -> Optional[str]:
        """Find TerrariaServer executable on Windows."""
        ...
        possible_paths = [
            r"C:\Program Files (x86)\Steam\steamapps\common\Terraria\TerrariaServer.exe",
            r"C:\Program Files\Steam\steamapps\common\Terraria\TerrariaServer.exe",
            os.path.expanduser(
                r"~\.local\share\Steam\steamapps\common\Terraria\TerrariaServer.exe"
            ),
        ]
    ...
    def find_world_directory(self) -> Optional[str]:
        """Find Terraria world directory on Windows."""
        # Check environment variable first
        if os.environ.get('TERRARIA_WORLD_DIR'):
            return os.environ['TERRARIA_WORLD_DIR']

        # Standard Windows path
        return os.path.expanduser(r"~\Documents\My Games\Terraria\Worlds")
    ...
    def generate_world(self, params: WorldGenerationParams) -> Optional[Path]:
        """
        Generate world using TerrariaServer on Windows.
        ...
        try:
            # Run TerrariaServer with input redirection
    ...
    def kill_terraria_server(self) -> bool:
        ...
        try:
            # Use taskkill to kill TerrariaServer.exe only
            subprocess.run(
                ['taskkill', '/F', '/IM', 'TerrariaServer.exe'],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
            return True
